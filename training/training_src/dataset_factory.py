...
class CustomDistributedIterableDataset(IterableDataset):
    
    def __init__(self, dataset_folder, label_columns=["label"], feature_columns=None, use_pyarrow=True, rank=0):
        # read metadata
        self.use_pyarrow = use_pyarrow
        if use_pyarrow:
            self.fs = pa.hdfs.connect('namenode', port=9000, driver='libhdfs3')
